defmodule TrailChronicleWeb.RaceLive.Index do
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        <%= if @loading do %>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-pulse">
            <%= for _ <- 1..6 do %>
              <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden h-96">
  ...
  def mount(_params, _session, socket) do
    ...
    # 1. Mount immediately with empty list and loading state
    # 2. Schedule async fetch
    if connected?(socket) do
      send(self(), :load_races)
    end
  ...
  @impl true
  def handle_info(:load_races, socket) do
    # This fetches the heavy data (including route_data) in background
    # Because it happens after mount, the UI is already visible
    races = Racing.list_races_summary(socket.assigns.athlete)
  ...
  def handle_info({:load_filtered_races, filter}, socket) do
    ...
    new_raw_races =
      case filter do
        ...
        "upcoming" -> Racing.list_upcoming_races_summary(athlete)
        _ -> Racing.list_races_summary(athlete)
      end
